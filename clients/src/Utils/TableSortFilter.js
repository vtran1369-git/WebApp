...
export default function App({columns, data}) {
    ...
    const setHiddenCols = () =>{
        const invisibleCols = ["IDcalEquip"];
    ...
    const handleRowSelected = (row, id) => {
        console.log(">>selectedRowID with IDcalEquip= ", id)
        row.isSelected? console.log("unseleted") : console.log(">>row selected")
        ...
        <table {...getTableProps()} className = "simple-table" size="small">
            ...
            return (
                <tr {...row.getRowProps()} 
                    ...
                    onClick = {() => handleRowSelected(row, row.original.IDcalEquip)}
                >
                    {row.cells.map(cell => {
                    return (
