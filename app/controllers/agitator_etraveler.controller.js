...
db.sequelize.query(`SELECT *, (SELECT concat(firstName," ", lastName) FROM people WHERE IDpeople = A.opID ) AS opFull_Name, (SELECT concat(firstName," ", lastName) FROM people WHERE IDpeople = A.qaID ) AS qaFull_Name, (SELECT concat(firstName," ", lastName) FROM people WHERE IDpeople = A.finalQAID ) AS finalQAFull_Name, (SELECT concat(firstName," ", lastName) FROM people WHERE IDpeople = A.editorID ) AS editedFull_Name from agitator_etraveler A
limit ${offset}, ${limit}`, {type: QueryTypes.SELECT})
